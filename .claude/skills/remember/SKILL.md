...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope spanish-cli
...
scope:** the default personal scope is **private** (`--scope spanish-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `spanish-cli`). Private records are served only to a recall in the same
