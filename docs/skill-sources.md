...
spanish-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. spanish-cli
...
#   - consumer-identifying prose: `guildmaster` → `spanish-cli` (NOT
...
If a re-sync would lose a spanish-cli adaptation, lift the change
...
#     → `which spanish-cli vendors from guildmaster`
